...
import {DTEs, noteDTEs} from 'app/services/libreDte/config'
import s3 from 'app/services/aws/s3'
import {checkField} from '../helpers'

export default {
  name: 'Libre DTE: Emitir Nota de Débito o Crédito',
  optionsSchema: {
    collectionId: {
      label: 'Collección',
      type: String,
    ...
    tasaIVA: {
      label: 'Tasa IVA. Debe ser un número entre 0 y 100',
      type: Number,
      fieldType: 'collectionFieldSelect',
      parentCollection: 'collectionId'
    },
    ...
    tpoDocRef: {
      label: 'Tipo de DTE al que hace referencia esta nota',
      type: Number,
      fieldType: 'select',
      fieldOptions: {options: DTEs}
    },
    folioRef: {
      label: 'Folio al que hace referencia la nota',
      type: Number
    },
    fchRef: {
      label: 'Fecha del documento de referencia. Formato: YYYY-MM-DD',
      type: String
    },
    codRef: {
      label:
        'Código de referencia. Ver http://www.sii.cl/factura_electronica/formato_dte.pdf y buscar "CodRef" para una expicación detallada',
      type: Number,
      fieldType: 'select',
      fieldOptions: {
        options: [
          {label: 'Anula documento de referencia', value: 1},
          {label: 'Corrige texto documento de referencia', value: 2},
    ...
    RazonRef: {
      label:
        'Razón de referencia. Explicitar razón. Ejemplo una Nota de Crédito que hacer referencia a una factura, indica "descuento por pronto pago" o "error en precio" etc.',
      type: String,
      fieldType: 'collectionFieldSelect',
      parentCollection: 'collectionId'
    },
  ...
  async execute({
    ...
    try {
      ...
      const Totales = {
        MntNeto: parseInt(mntNeto, 10),
        TasaIVA: parseInt(tasaIVA, 10),
        IVA: parseInt(iva, 10),
        MntTotal: parseInt(mntTotal, 10)
      }
      const Encabezado = {
        IdDoc,
        Emisor,
      ...
      const Referecia = {
        TpoDocRef: parseInt(tpoDocRef, 10),
        FolioRef: parseInt(folioRef, 10),
        FchRef: moment(fchRef, 'YYYY-MM-DD'),
        CodRef: parseInt(codRef, 10),
        RazonRef
      }
      const body = {
        Encabezado,
      ...
      const dteLabel = DTEs.filter(doc => doc.value == realDte.dte)[0].label
