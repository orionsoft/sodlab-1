...
export default {
  ...
  async execute({options, params}) {
    try {
      ...
      const masterProductsCollection = await Collections.findOne(
        options.maestroProductosCollectionId
      )
      ...
      const mapProducts = productsId.map(async product => {
        ...
        return {
          codigo: sku.data[options.productsSku],
      ...
      if (options.discountStock) {
        productsList.map(async product => {
          const query = {[`data.${options.productsSku}`]: product.codigo}
          const {data} = await masterProductsDB.findOne(query)
          if (product.cantidad > data.stock) {
            throw new Error(`Producto ${data.nombre} con stock de: ${data.stock}`)
          } else {
