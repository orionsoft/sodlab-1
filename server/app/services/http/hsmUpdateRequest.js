...
route('/hsm/update-requests', async function({getBody, ...params}) {
  ...
  const invalidHsmDocs = []
  const hsmDocuments = documents.map(async (doc, index) => {
    try {
      const hsmDoc = await HsmDocuments.findOne({itemId: doc.documentId, requestId: data.requestId})

      if (!doc.success) {
        invalidHsmDocs.push(doc.documentId)
  ...
  return {msg: `Hsm request ${data.requestId} processed ok`, invalidHsmDocs}
})
